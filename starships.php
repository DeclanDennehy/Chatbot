<?php
    ...
    function starships_query(){
        ...
        if ($count > 0){
            ...
            for($i = 0; ($i < $count-1 && $i < 9); $i++){
                $message .= '{"title":"'.$results["results"][$i]["name"].'","block_names":["Starship"],"set_attributes": {"starship_chosen": "'.$results["results"][$i]["url"].'"}},';
            }
            // add the contents of the last result and end the message. 
            $message .= '{"title":"'.$results["results"][$i]["name"].'","block_names":["Starship"],"set_attributes": {"starship_chosen": "'.$results["results"][$i]["url"].'"}}]}]}';
        } else {       
            // no results are found so indicate that to user and retry
            $message = '{"messages": [{"text": "No results are found"}],"redirect_to_blocks": ["Query Starships"]}';
        }
    ...
    /*
     *  Function to send a request for a starship page to the SWAPI and then return a description to the chatbot
     */
